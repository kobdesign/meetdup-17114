...
/**
 * Get LIFF ID for activation flow (tenant-specific)
 */
export async function getLiffIdActivation(tenantId: string): Promise<string | null> {
  const { data, error } = await supabaseAdmin
    .from("tenant_secrets")
    .select("liff_id_activation")
    .eq("tenant_id", tenantId)
    .single();

  if (error || !data || !data.liff_id_activation) {
    console.log(`[getLiffIdActivation] No LIFF ID found for tenant ${tenantId}, falling back to env var`);
    // Fallback to global LIFF ID from env var for backward compatibility
    return process.env.LIFF_ID || null;
  }

  try {
    return decryptValue(data.liff_id_activation);
  } catch (error) {
    console.error(`[getLiffIdActivation] Failed to decrypt LIFF ID for tenant ${tenantId}:`, error);
    return process.env.LIFF_ID || null;
  }
}

/**
 * Save LIFF ID for activation flow (tenant-specific)
 */
export async function saveLiffIdActivation(tenantId: string, liffId: string): Promise<void> {
  const encryptedLiffId = encryptValue(liffId);

  const { error } = await supabaseAdmin
    .from("tenant_secrets")
    .update({ liff_id_activation: encryptedLiffId })
    .eq("tenant_id", tenantId);

  if (error) {
    console.error(`[saveLiffIdActivation] Error saving LIFF ID for tenant ${tenantId}:`, error);
    throw new Error(`Failed to save LIFF ID: ${error.message}`);
  }

  console.log(`[saveLiffIdActivation] Successfully saved LIFF ID for tenant ${tenantId}`);
}
