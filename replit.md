...
- **LINE Integration**: Comprehensive multi-tenant LINE webhook system with destination-based tenant resolution, HMAC signature validation, secure credential management, rich menu management, and message-based interaction flows (e.g., phone linking, business card search, automated LIFF activation). Uses an Express-only webhook at `/api/line/webhook`.
...
- **Activation Flow**: Secure token-based self-registration for imported members with single-use, 7-day expiring tokens, facilitating account creation and linking.
